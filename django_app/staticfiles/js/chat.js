document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAttachmentLock() {
        ...
        if (hasText) {
            // 텍스트 입력 시 이미지 첨부 비활성 + 기존 이미지 제거
            if (uploadedFile || uploadedImage) {
                clearImageSelection();
            }
            btnUploadImage.disabled = true;
        } else {
            if (!isSending && !uploadedFile) btnUploadImage.disabled = false;
        }

        if (uploadedFile || uploadedImage) {
            chatInput.disabled = true;
            chatInput.placeholder = '이미지 단독 전송만 가능합니다.';
        } else if (!isSending) {
            chatInput.disabled = false;
            chatInput.placeholder = '인테리어 고민을 물어보세요...';
        }
    ...
    btnUploadImage.addEventListener('click', () => { if (Auth.requireLogin()) fileInput.click(); });
